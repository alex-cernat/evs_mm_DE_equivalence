...
DT::datatable(cfa_fit) %>% 
  DT::formatRound(columns = c(colnames(cfa_fit)), digits = 2)
...
```{r}
DT::datatable(fit_eq) %>% 
  DT::formatRound(columns = c(colnames(fit_eq)), digits = 2)
```
...
```{r}
sig_parameters %>% 
  mutate(dif = est_2 - est_1) %>% 
  DT::datatable() %>% 
  DT::formatRound(columns = c(colnames(sig_parameters), "dif"), digits = 2)
```
